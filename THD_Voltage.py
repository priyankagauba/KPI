...
class THD_Voltage():
    ...
    def categorization_time(self,df):
        ...
        df['Status'] = np.vectorize(d.get)(np.digitize(df['EM_THD Voltage'], bins))
        return df
    ...
    def time(self,df):
        ...
        a = df.loc[list(df.groupby('DeviceID')['EM_THD Voltage'].idxmax())][['Time_max','DeviceID']]
        b = df.loc[list(df.groupby('DeviceID')['EM_THD Voltage'].idxmin())][['Time_max','DeviceID']]
        a.index = a['DeviceID']
    ...
    def output(self):
        ...
        df0 = df[df['EM_THD Voltage'] == 0]
        df = df[df['EM_THD Voltage'] > 0]
        x = df.groupby('DeviceID')['EM_THD Voltage'].describe()
        ...
        print(self.DFDBClient.write_points(x, cg.THD_VOLTAGE))
        return x    
