...
class THD_Current1():
    ...
    def categorization_time(self,df):
        ...
        df['Status'] = np.vectorize(d.get)(np.digitize(df['EM_THD Current1'], bins))
        return df
    ...
    def time(self,df):
        ...
        a = df.loc[list(df.groupby('DeviceID')['EM_THD Current1'].idxmax())][['Time_max','DeviceID']]
        b = df.loc[list(df.groupby('DeviceID')['EM_THD Current1'].idxmin())][['Time_max','DeviceID']]
        a.index = a['DeviceID']
    ...
    def output(self):
        ...
        df0 = df[df['EM_THD Current1'] == 0]
        df = df[df['EM_THD Current1'] > 0]
        x = df.groupby('DeviceID')['EM_THD Current1'].describe()
        ...
        print(self.DFDBClient.write_points(x, cg.THD_CURRENT1))
        return x    
