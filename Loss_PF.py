...
class PFLoss():
    ...
    def total_sum(self,df):
        y = pd.DataFrame(df.groupby('DeviceID')['kvah_loss_pf'].sum()).astype(np.float64)
        y.columns = ['Sum']
        y = y.reset_index()
        return y 
    ...
    def output(self):
        df = self.read_Data()
        x = df.groupby('DeviceID')['kvah_loss_pf'].describe()
        ...
        print(self.DFDBClient.write_points(x, cg.PF_LOSS))
        return x
