...
class C2():
    ...
    def avg_stability(self, df):
        df['diff'] = abs(df.groupby('DeviceID')['EM_Current Ph2 (A)'].diff())
        df = df.fillna(0)
    ...
    def time(self, df):
        ...
        a = df.loc[list(df.groupby('DeviceID')['EM_Current Ph2 (A)'].idxmax())][['Time_max', 'DeviceID']]
        b = df.loc[list(df.groupby('DeviceID')['EM_Current Ph2 (A)'].idxmin())][['Time_max', 'DeviceID']]
        a.index = a['DeviceID']
    ...
    def categorization_time(self, df):
        ...
        for i in self.a.keys():
            x = df[df['DeviceID'] == i]
            x['Status'] = np.where(x['EM_Current Ph2 (A)'] > self.a[i], 'high', x['Status'])
            df[df['DeviceID'] == i] = x
        return df
    ...
    def summary_data(self, df):
        x = df.groupby('DeviceID')['EM_Current Ph2 (A)'].describe()
    ...
    def output(self):
        ...
        df0 = df[df['EM_Current Ph2 (A)'] == 0]
        df = df[df['EM_Current Ph2 (A)'] > 0]
        ...
        print(self.DFDBClient.write_points(x, cg.CurrentL2))
        return x
