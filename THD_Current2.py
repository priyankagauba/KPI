...
class THD_Current2():
    ...
    def categorization_time(self,df):
        ...
        df['Status'] = np.vectorize(d.get)(np.digitize(df['EM_THD Current2'], bins))
        return df
    ...
    def time(self,df):
        ...
        a = df.loc[list(df.groupby('DeviceID')['EM_THD Current2'].idxmax())][['Time_max','DeviceID']]
        b = df.loc[list(df.groupby('DeviceID')['EM_THD Current2'].idxmin())][['Time_max','DeviceID']]
        a.index = a['DeviceID']
    ...
    def output(self):
        ...
        df0 = df[df['EM_THD Current2'] == 0]
        df = df[df['EM_THD Current2'] > 0]
        x = df.groupby('DeviceID')['EM_THD Current2'].describe()
        ...
        print(self.DFDBClient.write_points(x, cg.THD_CURRENT2))
        return x    
