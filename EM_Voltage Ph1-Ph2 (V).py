...
class V12():
    ...
    def avg_stability(self, df):
        df['diff'] = abs(df.groupby('DeviceID')['EM_Voltage Ph1-Ph2 (V)'].diff())
        df = df.fillna(0)
    ...
    def time(self, df):
        ...
        a = df.loc[list(df.groupby('DeviceID')['EM_Voltage Ph1-Ph2 (V)'].idxmax())][['Time_max', 'DeviceID']]
        b = df.loc[list(df.groupby('DeviceID')['EM_Voltage Ph1-Ph2 (V)'].idxmin())][['Time_max', 'DeviceID']]
        a.index = a['DeviceID']
    ...
    def categorization_count(self,df):
        ...
        df['Status'] = np.vectorize(d.get)(np.digitize(df['EM_Voltage Ph1-Ph2 (V)'], bins))
        return df
    ...
    def summary_data(self, df):
        x = df.groupby('DeviceID')['EM_Voltage Ph1-Ph2 (V)'].describe()
    ...
    def output(self):
        ...
        df0 = df[(df['EM_Voltage Ph1-Ph2 (V)'] == 0)]
        df = df[(df['EM_Voltage Ph1-Ph2 (V)'] > 0)]
        ...
        print(self.DFDBClient.write_points(x, cg.VoltageP12))
        return x
