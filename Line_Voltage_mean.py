...
class MeanLineVoltage():
    ...
    def avg_stability(self, df):
        df['diff'] = abs(df.groupby('DeviceID')['mean_voltage'].diff())
        df = df.fillna(0)
    ...
    def time(self, df):
        ...
        a = df.loc[list(df.groupby('DeviceID')['mean_voltage'].idxmax())][['Time_max', 'DeviceID']]
        b = df.loc[list(df.groupby('DeviceID')['mean_voltage'].idxmin())][['Time_max', 'DeviceID']]
        a.index = a['DeviceID']
    ...
    def categorization_count(self,df):
        ...
        df['Status'] = np.vectorize(d.get)(np.digitize(df['mean_voltage'], bins))
        return df
    ...
    def summary_data(self, df):
        x = df.groupby('DeviceID')['mean_voltage'].describe()
    ...
    def output(self):
        ...
        df0 = df[(df['mean_voltage'] == 0)]
        df = df[(df['mean_voltage'] > 0)]
        ...
        print(self.DFDBClient.write_points(x, cg.Line_Voltage_Mean))
        return x
