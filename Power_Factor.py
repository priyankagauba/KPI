...
class PowerFactor():
    ...
    def categorization_time(self,df):
        bins = [0, 0.8, 0.85, np.inf]
        names = ['Time_Critical', 'Time_Warning', 'Time_Good']
        d = dict(enumerate(names, 1))
        df['Status'] = np.vectorize(d.get)(np.digitize(df['EM_Power Factor'], bins))
        return df
    ...
    def categorization_count(self,df):
        bins = [0, 0.8, 0.85, np.inf]
        names = ['Count_Critical', 'Count_Warning', 'Count_Good']
        d = dict(enumerate(names, 1))
        df['Status'] = np.vectorize(d.get)(np.digitize(df['EM_Power Factor'], bins))
        return df
    ...
    def time(self,df):
        ...
        a = df.loc[list(df.groupby('DeviceID')['EM_Power Factor'].idxmax())][['Time_max','DeviceID']]
        b = df.loc[list(df.groupby('DeviceID')['EM_Power Factor'].idxmin())][['Time_max','DeviceID']]
        a.index = a['DeviceID']
    ...
    def output(self):
        ...
        df0 = df[df['EM_Power Factor'] == 0]
        df = df[df['EM_Power Factor'] > 0]
        x = df.groupby('DeviceID')['EM_Power Factor'].describe()
        ...
        print(self.DFDBClient.write_points(x, cg.POWER_FACTOR))
        return x    
