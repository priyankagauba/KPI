...
class Voltagel3():
    ...
    def avg_stability(self, df):
        df['diff'] = abs(df.groupby('DeviceID')['EM_Voltage Ph3-N (V)'].diff())
        df = df.fillna(0)
    ...
    def time(self, df):
        ...
        a = df.loc[list(df.groupby('DeviceID')['EM_Voltage Ph3-N (V)'].idxmax())][['Time_max', 'DeviceID']]
        b = df.loc[list(df.groupby('DeviceID')['EM_Voltage Ph3-N (V)'].idxmin())][['Time_max', 'DeviceID']]
        a.index = a['DeviceID']
    ...
    def categorization_count(self,df):
        bins = [0, 195, 205, 240, 245, np.inf]
        names = ['count_critical', 'count_warning', 'count_normal', 'count_warning', 'count_critical']
        d = dict(enumerate(names, 1))
        df['Status'] = np.vectorize(d.get)(np.digitize(df['EM_Voltage Ph3-N (V)'], bins))
        return df
    ...
    def summary_data(self, df):
        x = df.groupby('DeviceID')['EM_Voltage Ph3-N (V)'].describe()
    ...
    def output(self):
        ...
        df0 = df[(df['EM_Voltage Ph3-N (V)'] == 0)]
        df = df[(df['EM_Voltage Ph3-N (V)'] > 0)]
        ...
        print(self.DFDBClient.write_points(x, cg.VoltageL3))
        return x
