...
class THD_Current3():
    ...
    def categorization_time(self,df):
        ...
        df['Status'] = np.vectorize(d.get)(np.digitize(df['EM_THD Current3'], bins))
        return df
    ...
    def time(self,df):
        ...
        a = df.loc[list(df.groupby('DeviceID')['EM_THD Current3'].idxmax())][['Time_max','DeviceID']]
        b = df.loc[list(df.groupby('DeviceID')['EM_THD Current3'].idxmin())][['Time_max','DeviceID']]
        a.index = a['DeviceID']
    ...
    def output(self):
        ...
        df0 = df[df['EM_THD Current3'] == 0]
        df = df[df['EM_THD Current3'] > 0]
        x = df.groupby('DeviceID')['EM_THD Current3'].describe()
        ...
        print(self.DFDBClient.write_points(x, cg.THD_CURRENT3))
        return x    
