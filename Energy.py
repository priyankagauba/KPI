...
class Energy():
    ...
    def energy_per_day(self, df):
        y = pd.DataFrame(df.groupby(['DeviceID'])['EM_Energy (kWh)'].apply(lambda x:x.iloc[-1]-x.iloc[0]))
        y.rename(columns={"DeviceID": "DeviceID", "EM_Energy (kWh)": "EPD"}, inplace = True)
    ...
    def output(self):
        df = self.read_Data()
        print(df)
        x = df.groupby('DeviceID')['EM_Energy (kWh)'].describe()
        ...
        print(x)
        print(self.DFDBClient.write_points(x, cg.ENERGY))
        return x 
