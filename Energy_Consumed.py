...
class EnergyConsumed():
    ...
    def energy_per_day(self, df):
        y = pd.DataFrame(df.groupby(['DeviceID'])['EM_TOTAL_Import_Energy(kWh)'].apply(lambda x:x.iloc[-1]-x.iloc[0]))
        y.rename(columns={"DeviceID": "DeviceID", "EM_TOTAL_Import_Energy(kWh)": "EPD"}, inplace = True)
        y.reset_index(inplace=True)
        return y
    
    def output(self):
        df = self.read_Data()
        x = df.groupby('DeviceID')['EM_TOTAL_Import_Energy(kWh)'].describe()
        ...
        print(self.DFDBClient.write_points(x, cg.ENERGY_CONSUMED))
        return x 
